...
public class ConnectionManager implements IConnectionManager,
    ...
    private static BlockingQueue<Node> nodesQueue = null;
    
    public static void disconnectNodes(Set<Node> nodes) {
    	if ( nodes == null || nodesQueue == null) {
    		return;
    	}
    	for( Node node: nodes) {
    		try {
    ...
    public void startForLB() {
    	nodesQueue = new LinkedBlockingDeque<Node>();
...
//                        connectionService.notifyClusterViewChanged();
    ...
    public void _hehe(CommandInterpreter ci) {
//        connectionEventThread.interrupt();
        Set<Node> localNodes = getLocalNodes();
        if (localNodes != null) {
//            AbstractScheme scheme = schemes.get(activeScheme);
...
//                    scheme.removeNode(localNode);
    ...
    public void _xixi(CommandInterpreter ci) {
